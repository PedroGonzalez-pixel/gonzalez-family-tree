...
function drawTree(P){
  ...
  // ─────────────────────────────────────────────────────────
  // PATCH : recentrage des enfants sous le SLOT parent
  // (SEUL CHANGEMENT par rapport à la v2.6.1)
  // ─────────────────────────────────────────────────────────
  Object.values(fams).forEach(f=>{
    ...
    const children=f.ch.map(id=>pos[id]).filter(Boolean);
    if(!children.length) return;

    const totalWidth =
      children.length*NW+(children.length-1)*HGAP;

    let cx=parentCenterX-totalWidth/2;
    children.forEach(c=>{
      c.x=cx;
      cx+=NW+HGAP;
    });
  ...
  Object.keys(spouseOf).forEach(a=>{
    ...
    const y=pa.y+NH/2;
    const x1=Math.min(pa.x,pb.x)+NW;
  ...
  Object.values(fams).forEach(f=>{
    ...
    if(pf)
      g.append("path")
        .attr("d",`M${pf.x+NW/2},${pY} V${jY} H${cx}`)
        .attr("stroke","#c0c0c8").attr("fill","none");

    if(pm&&pm!==pf)
      g.append("path")
        .attr("d",`M${pm.x+NW/2},${pY} V${jY} H${cx}`)
        .attr("stroke","#c0c0c8").attr("fill","none");

    f.ch.forEach(id=>{
      const c=pos[id]; if(!c) return;
      g.append("line")
        .attr("x1",cx).attr("y1",jY)

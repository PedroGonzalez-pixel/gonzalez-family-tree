// ARBRE GÉNÉALOGIQUE v2.6.2
const TREE_VERSION = "2.6.2";
...
function drawTree(P){
  ...
  /* ✅ RECENTRAGE DES ENFANTS SOUS LEURS PARENTS */
  Object.values(fams).forEach(f=>{
    ...
    const children=f.ch.filter(id=>pos[id]).map(id=>pos[id]);
    if(!children.length)return;

    const totalWidth=children.length*NW+(children.length-1)*HGAP;
  ...
  svg.append("text").attr("x",10).attr("y",18).attr("font-size",10).attr("fill","#aaa")
    .text("v"+TREE_VERSION);
  ...
  Object.values(fams).forEach(f=>{
    ...
    const centerX=pf&&pm?(pf.x+pm.x+NW)/2:pf?pf.x+NW/2:pm.x+NW/2;
    const pY=(pf||pm).y+NH;
    const jY=pY+VGAP*0.4;

    if(pf)g.append("path").attr("d",`M${pf.x+NW/2},${pY}V${jY}H${centerX}`)
      .attr("stroke","#c0c0c8").attr("fill","none");
    if(pm&&pm!==pf)g.append("path").attr("d",`M${pm.x+NW/2},${pY}V${jY}H${centerX}`)
      .attr("stroke","#c0c0c8").attr("fill","none");

    f.ch.forEach(id=>{
      const c=pos[id];if(!c)return;
      g.append("line")
        .attr("x1",centerX).attr("y1",jY)
        .attr("x2",c.x+NW/2).attr("y2",c.y)
        .attr("stroke","#c0c0c8");
    });
  });
  ...
  ids.forEach(id=>{
    ...
    const n=g.append("g").style("cursor","pointer")
      .on("click",()=>location.href="person.html?id="+id);

    n.append("rect").attr("x",pt.x).attr("y",pt.y)
      .attr("width",NW).attr("height",NH).attr("rx",10)
      .attr("fill",p.dd?"#f2f2f4":"white").attr("stroke","#d1d1d6");

    n.append("text").attr("x",pt.x+NW/2).attr("y",pt.y+30)
      .attr("text-anchor","middle").attr("font-size",12).text(p.n);

    const inf=getInfo(p);
    if(inf)n.append("text").attr("x",pt.x+NW/2).attr("y",pt.y+48)

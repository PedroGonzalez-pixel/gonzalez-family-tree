...
function drawTree(P){
  ...
  Object.values(fams).forEach(f=>{
    ...
    const totalW = children.length * NW + (children.length - 1) * HGAP;
    let cx = parentCenterX - totalW/2;
  ...
  Object.values(fams).forEach(f=>{
    ...
    if(pf)
      g.append("path")
        .attr("d", `M${pf.x+NW/2},${pY} V${jY} H${centerX}`)
        .attr("stroke", "#c0c0c8")
        .attr("fill", "none");

    if(pm && pm !== pf)
      g.append("path")
        .attr("d", `M${pm.x+NW/2},${pY} V${jY} H${centerX}`)
        .attr("stroke", "#c0c0c8")
        .attr("fill", "none");
